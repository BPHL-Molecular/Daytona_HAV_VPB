process pystats {
    ...
    with open(filepath3, 'r') as kreport:
        ...
        for l in lines:
            ...
            substring1 = "Hepatovirus A"
            
            if (tax_level == 'S') and (substring1.lower() in tax.lower()):
                toge=tax+"|"+tax_id+"|"+str(percent)+"|"+str(num_reads)
                taxes.append(toge)
    taxex_csv = ','.join(taxes)

    with open("${mypath}"+"/report.txt", 'w') as report:
        header = ['sampleID', 'species|tax_ID|percent(%)|number', 'reference', 'start', 'end', 'num_raw_reads', 'num_clean_reads', 'num_mapped_reads', 'percent_mapped_clean_reads', 'cov_bases_mapped', 'percent_genome_cov_map', 'mean_depth', 'mean_base_qual', 'mean_map_qual']
